...
//    private String[][] generatePlayfairMatrix(String key) {
...
//        }
//
//        // Convert byte matrix to String matrix using Windows-1251 encoding
//        String[][] stringMatrix = new String[MATRIX_ROWS][MATRIX_COLUMNS];
//        for (int row = 0; row < MATRIX_ROWS; row++) {
//            for (int col = 0; col < MATRIX_COLUMNS; col++) {
//                stringMatrix[row][col] = new String(new byte[]{matrix[row][col]}, ENCODING);
//            }
//        }
//        return stringMatrix;
//    }
...
private char[][] generatePlayfairMatrix(String key) {
    ...
    for (byte c : key.getBytes(ENCODING)) {
        int d = Byte.toUnsignedInt(c);
        if ((d >= ASCII_START && d <= ASCII_END || d == 9 || d == 10 || d == 13) && !used[d]) {
            uniqueBytes.add(c);
            used[d] = true;
        }
    }

    // Add special ASCII characters 9, 10, 13 explicitly if missing
    for (int special : new int[]{9, 10, 13}) {
        if (!used[special]) {
            uniqueBytes.add((byte) special);
            used[special] = true;
        }
    }
